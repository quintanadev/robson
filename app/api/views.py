...
from django.db.models import DateTimeField, DateField, CharField, IntegerField, Value, Case, When, Count, Sum, Max

from services.data_integrator.nicecxone.realtime import get_sla_skill_summary, get_forecast
from .models import NiceContact
...
def inbound_cards(request):
  ...
  nice_contacts_rec = (
    NiceContact.objects
    .annotate(
      startDateCast=Cast(Cast('contactStart', output_field=DateTimeField()) + timedelta(hours=-3), output_field=DateField()),
      startDateTimeCast=Cast('contactStart', output_field=DateTimeField()) + timedelta(hours=-3),
      startMinuteCast=Cast(Substr(Cast(Cast('contactStart', output_field=DateTimeField()) + timedelta(hours=-3), output_field=CharField()), 15, 2), output_field=IntegerField()),
      intervalCast=Cast(Concat(
        Substr(Cast(Cast('contactStart', output_field=DateTimeField()) + timedelta(hours=-3), output_field=CharField()), 12, 2),
        Case(
          When(startMinuteCast__gte=30, then=Value('30')),
          default=Value('00')
        )
      ), output_field=IntegerField()),
      timeCast=Replace(Substr(Cast(Cast('contactStart', output_field=DateTimeField()) + timedelta(hours=-3), output_field=CharField()), 12, 8), Value(':'), Value(''))
    )
    .filter(
      campaignName__in=['RECEPTIVO', 'COB - RECEPTIVO DIGITAL'],
      startDateCast__in=[data_atual, data_comparativo],
      # intervalCast__lte=int(intervalo_atual[:5].replace(':', ''))
      timeCast__lte=int(data_atualizacao.strftime("%H%M%S"))
    )
  ).values('skillName', 'startDateCast').order_by().annotate(
    volume=Count('skillName'),
    abandonadas=Sum(Case(When(abandoned='True', then=Value(1)), default=Value(0))),
    atendidas=Sum(Case(When(abandoned='False', then=Value(1)), default=Value(0))),
    # target=Sum(),
    # negocios=Sum(),
    tempoAtendimento=Sum('agentSeconds'),
    dataAtualizacao=Max('startDateTimeCast')
  )
  df_db = pd.DataFrame.from_records(
    nice_contacts_rec.values_list('skillName', 'startDateCast', 'volume', 'abandonadas', 'atendidas', 'tempoAtendimento', 'dataAtualizacao'),
    columns=[col for col in nice_contacts_rec[0].keys()]
  )
