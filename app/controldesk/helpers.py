...
def get_skill_agents_logged(skill, headers, updated_since):
  api_link = f'skills/{skill}/agents'
  query = '?updatedSince={}&fields={}'
  api = api_link + query.format(updated_since, 'agentId,firstName,isActive,isAssigned,isSkillActive,isDialer,isOutbound,skillId,skillName')
...
def get_agents_status(headers, updated_since):
  api_link = 'agents/states'
  query = '?top={}&fields={}&updatedSince={}'
  api = api_link + query.format('10000', 'agentId,agentStateId,agentStateName,isActive,isACW,isOutbound,contactId', updated_since)
  print(f"Executando api: {api_link}")
  api_url = urls()['cxone-v30'] + api
  columns = []
  data = []
